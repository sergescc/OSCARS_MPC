...
public class DemoGuiController 
{
    ...
    * @return The list of associated subrequest GRIs as an array of Objects
    *******************************************************************************************************/
	public ArrayList<String> getUngroupedGRIs(ArrayList<String> mpGriList) {
		// TODO Auto-generated method stub
		//allUnicastGris
		ArrayList<String> allGroupedGris = new ArrayList<String>();
		ArrayList<String> allUngroupedGris = new ArrayList<String>();
		for(int i = 0; i < mpGriList.size();i++){
			Object[] groupedGris = getGroupedGRIs(mpGriList.get(i));
			for(int j = 0; j < groupedGris.length; j++){
				allGroupedGris.add(groupedGris[j].toString());
			}
		}
		for(int i = 0; i < allUnicastGris.size(); i++){
			if(!allGroupedGris.contains(allUnicastGris.get(i))){ //if it's a unicast GRI and is not contained in a group, add it to the ungrouped GRIs list
				allUngroupedGris.add(allUnicastGris.get(i));
			}
		}
		return allUngroupedGris;
	}
    ...
    {	
		...
		if(uniResConst != null)
		{
			...
			for(int h = 0; h < hops.size() - 1; h++)
			{
				...
				String currURN = currHop.getLink().getId();
				String nextURN = nextHop.getLink().getId();
				
				String currNode = currURN.substring(currURN.indexOf("node=") + 5, currURN.indexOf(":port="));
				String nextNode = nextURN.substring(nextURN.indexOf("node=") + 5, nextURN.indexOf(":port="));
				
				String currPort = currURN.substring(currURN.indexOf("port=") + 5, currURN.indexOf(":link="));
				String nextPort = nextURN.substring(nextURN.indexOf("port=") + 5, nextURN.indexOf(":link="));
				
				String currLink = currURN.substring(currURN.indexOf("link=") + 5);
				String nextLink = nextURN.substring(nextURN.indexOf("link=") + 5);
				
				
				
				if(!currNode.equals(nextNode))
				{
					allHops += currNode + "_" + nextNode + "_" + currPort + "_" + nextPort;
